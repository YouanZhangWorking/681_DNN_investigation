...
class SSD(object):
    _defaults = {
        "model_path"        : r'C:\Users\5109I21112\Documents\clone\681_DNN_investigation\keras\detection\SSD_ipynb_transfer_callback\output\20230819\good_detection_test_callback_2.h5',
        "classes_path"      : r'C:\Users\5109I21112\Documents\clone\681_DNN_investigation\keras\detection\SSD_ipynb_transfer_callback\output\20230819\voc_classes.txt',
        "input_shape"       : [120, 160],
        #---------------------------------------------------------------------#
        #   只有得分大于置信度的预测框会被保留下来
        #---------------------------------------------------------------------#
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        # 用于h5模型的数据
        image_data_h5 = image_data / 127.5 - 1.0
        # h5预测得到结果
        preds = self.ssd.predict(image_data_h5)
        # print("------",preds/0.07807575166225433-8)
        # 用于tflite的数据
        image_data_tf = image_data.astype('float32') -128.5
        image_data_tf = np.int8(image_data_tf)
        ...
        # 加载模型
        # model_path = r"C:\Users\5109I21112\Documents\clone\681_DNN_investigation\keras\detection\SSD_ipynb_transfer_callback\output\20230810\good_detection_test_callback_2.h5"
        ...
        # 推理
        interpreter.set_tensor(input_details[0]['index'],image_data_tf)
        interpreter.invoke()
        ...
        # tflite输出逆运算
        preds2 = 0.07807575166225433 * (preds2 + 8)
        ...
        #   将预测结果进行解码
        #-----------------------------------------------------------#
        results     = self.bbox_util.decode_box(preds2, self.anchors, image_shape, 
