...
def generate_frame(size=(120,160),r_min=10,r_max=20,h_min=20,h_max=40, w_min=20, w_max=40,pad=1.0,border=20):
    ...
    if random.random() <= 0.5:
        ...
        x = random.randint(r+border,int(size[1]-pad*r*2)-border)
        y = random.randint(r+border,int(size[0]-pad*r*2)-border)
        image = cv2.circle(image,(x,y),r,(intensity),-1)
        bbox = [0,y/size[0],x/size[1],(2*r)/size[0],(2*r)/size[1]]
    else:
        ...
        bbox = [1,yc/size[0],xc/size[1],h/size[0],w/size[1]]
