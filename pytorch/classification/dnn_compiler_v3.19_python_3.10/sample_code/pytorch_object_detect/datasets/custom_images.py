...
class CustomImagesDataset:
    ...
    def __init__(self, root, transform=None, target_transform=None, is_yolo=False, is_eval=False, class_names=None, is_background_in_labels = False, is_gray=True):
        ...
        self.yolo_annotation_format = is_yolo  # True
    ...
    def _get_annotation(self, image_id):
        ...
        with open(annotation_file, 'r') as f:
            for line in f:
                class_id, x1, y1, x2, y2 = line.split()  # [x1, y1, x2, y2] <-- [y_center, x_center, h, w]
                bbox = [float(x1), float(y1), float(x2), float(y2)]
                if self.yolo_annotation_format: # True
    ...
    def _get_pascal_voc_format(self, boxes, img_size):  # [x1, y1, x2, y2] <-- [y_center, x_center, h, w]

        if self.is_gray:
           dh, dw = img_size
        else:
           dh, dw, c = img_size
        xc, yc, w, h = boxes  # [x_center, y_center, w, h] <-- [y_center, x_center, h, w]
        x1 = int(float((xc-w/2) * dw))
        x2 = int(float((xc+w/2) * dw))
        y1 = int(float((yc-h/2) * dh))
        y2 = int(float((yc+h/2) * dh))
        converted_bboxes = [x1, y1, x2, y2]  # [x1, y1, x2, y2] <-- [y1, x1, y2, x2]
